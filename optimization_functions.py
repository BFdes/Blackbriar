...
def stepint(dimension, variables):
    return 25.0+sum(math.floor(variables[i]) for i in xrange(1,dimension+1))
#a discretization of the sphere function
#domain is [-5.12,5.12] at f=0
def stepS(dimension, variables):
    return sum((math.floor(variables[i]+0.5))**2 for i in xrange(1,dimension+1))
#quartic function
...
def quartic(dimension, variables):
    a=sum(i*(variables[i]**4) for i in xrange(1,dimension+1))
